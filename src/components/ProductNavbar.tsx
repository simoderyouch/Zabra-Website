    ...
    return (
        <div className="absolute top-24 left-0 right-0 z-40 flex justify-center pointer-events-none mt-2 transition-all duration-300 hidden md:flex">
            <div className="bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-white/50 rounded-full h-14 px-8 flex items-center justify-center gap-10 pointer-events-auto transition-transform duration-500 hover:bg-white/95 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                <ProductLink href="/products/olive-oil" label={t('productNav.oliveOil')} />
                <ProductLink href="/products/argan-oil" label={t('productNav.arganOil')} />
                <ProductLink href="/products/dates" label={t('productNav.mejhoulDates')} />
                <ProductLink href="/products/gift" label={t('productNav.giftSets')} />
            </div>
        </div>
    );
}
...
function ProductLink({ href, label }: { href: string; label: string }) {
    return (
        <Link href={href} className="relative group py-1">
            <span className="text-[14px] lg:text-[15px] font-extrabold text-zabra-dark tracking-wider uppercase group-hover:text-[rgba(212,175,55,0.9)] transition-colors duration-300">
                {label}
            </span>
